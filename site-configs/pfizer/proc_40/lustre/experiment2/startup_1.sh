...
resultdir=$ROOT/result # the directory where the output will be kept
...
function setup_directories {
  ...
  io500_workdir=$datadir/$timestamp # directory where the data will be stored
  io500_result_dir=$resultdir/$timestamp      # the directory where the output results will be kept
  mkdir -p $io500_workdir $io500_result_dir

  mkdir -p ${io500_workdir}/ior_easy
  lfs setstripe --stripe-count 4 $io500_workdir/ior_easy


  mkdir -p ${io500_workdir}/ior_hard
  lfs setstripe --stripe-count 8 $io500_workdir/ior_hard
}
...
function setup_ior_easy {
  io500_ior_easy_params="-t 1024k -b 16G -F" # 2M writes, 16 GB per proc, file per proc 
}

function setup_mdt_easy {
  io500_mdtest_easy_params="-u -L" # unique dir per thread, files only at leaves
  io500_mdtest_easy_files_per_proc=15000
}
...
function setup_mdt_hard {
  io500_mdtest_hard_files_per_proc=3000
}
...
function run_benchmarks {
  ...
  source $ROOT/bin/io500_fixed.sh 2>&1 | tee $resultdir/$timestamp/io-500-summary.txt
}
